...
export default function ServiciosLogsPage() {
  ...
  async function load(current: ObservabilityEventsParams) {
    setLoading(true);
    setError(null);
    const [summaryResponse, eventsResponse] = await Promise.all([
      apiClient.getObservabilitySummary(),
      apiClient.listObservabilityEvents(current),
    ]);
  ...
  async function openDetail(id: string) {
    ...
    setOpen(true);
    const response = await apiClient.getObservabilityEvent(id);
    if (response.success) {
  ...
  return (
    <div className="flex-1 space-y-6 p-4 pt-6 md:p-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard icon={Activity} label="Eventos 24h" value={summary?.totals.events24h ?? 0} note={`${summary?.totals.totalEvents ?? 0} acumulados`} />
        <MetricCard icon={AlertTriangle} label="Errores 24h" value={summary?.totals.errorEvents24h ?? 0} note={`${summary?.totals.requestFailures24h ?? 0} requests 5xx`} />
        <MetricCard icon={ShieldCheck} label="Acciones admin" value={summary?.totals.adminActions24h ?? 0} note="mutaciones humanas" />
        <MetricCard icon={Activity} label="Emails 24h" value={summary?.totals.emailEvents24h ?? 0} note={`${summary?.totals.slowRequests24h ?? 0} requests lentos`} />
      </div>

      <Card className="border-zinc-200 shadow-sm">
        <CardHeader>
          <CardTitle>Filtros</CardTitle>
          <CardDescription>Busca por requestId, entidad, actor o dominio.</CardDescription>
        </CardHeader>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.8fr_1fr]">
              ...
              <>
                <div className="max-h-[720px] overflow-auto">
                  <Table>
                    ...
                    <TableBody>
                      {events.map((event) => (
                        <TableRow key={event.id} className={`cursor-pointer ${selectedId === event.id ? "bg-emerald-50/60" : ""}`} onClick={() => void openDetail(event.id)}>
                          ...
                          <TableCell className="text-sm text-zinc-600">{event.entityType || "n/a"}</TableCell>
        ...
        <div className="space-y-4">
          <Card className="border-zinc-200 shadow-sm">
            <CardHeader>
              <CardTitle>Dominios activos</CardTitle>
              <CardDescription>Lo que mas esta emitiendo eventos ahora.</CardDescription>
            </CardHeader>
          ...
          <Card className="border-zinc-200 shadow-sm">
            <CardHeader>
              <CardTitle>Warnings y fallos</CardTitle>
              <CardDescription>Los ultimos eventos para revisar.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {summary?.recentFailures?.map((event) => (
                <button key={event.id} className="w-full rounded-2xl border border-zinc-200 px-4 py-3 text-left hover:border-emerald-300 hover:bg-emerald-50/40" onClick={() => void openDetail(event.id)}>
                  <div className="flex flex-wrap gap-2">
                    <Badge className={`border ${tone(event.status)}`}>{event.status}</Badge>
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-2xl">
          <SheetHeader>
            ...
            <SheetDescription>Diff sanitizado, metadata y timeline correlacionado.</SheetDescription>
          </SheetHeader>
