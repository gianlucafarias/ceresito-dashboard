  ...
  try {
    ...
    if (!cuadrilla) {
      return NextResponse.json(
        { error: "No se encuentra la cuadrilla" },
        { status: 404 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Error al buscar la cuadrilla" },
      { status: 500 },
    );
  }
}
...
export async function PATCH(
  ...
  if (data.limiteReclamosSimultaneos !== undefined)
    updateData.limiteReclamosSimultaneos = data.limiteReclamosSimultaneos;
