...
export async function POST(request: Request) {
  try {
    ...
    console.log('Datos recibidos:', { reclamoId, cuadrillaId, reclamoDetalles });
    ...
    const reclamosActuales = cuadrilla.reclamosAsignados.length;

    if (reclamosActuales >= cuadrilla.limiteReclamosSimultaneos) {
      console.error('La cuadrilla ha alcanzado su límite de reclamos simultáneos');
      return NextResponse.json(
        { error: 'La cuadrilla ha alcanzado su límite de reclamos simultáneos' },
        { status: 400 }
      );
    }
    ...
    const response = await fetch(`https://api.ceres.gob.ar/api/api/reclamos/${reclamoId}`, {
      ...
      body: JSON.stringify({ estado: 'ASIGNADO', cuadrillaId: cuadrillaId, prioridad: "Media" }),
    });

    await prisma.mensaje.create({
        data: {
          contenido: `NUEVO RECLAMO. Reclamo #${reclamoId} ha sido asignado a la cuadrilla.`,
          remitente: 'Usuario',
          cuadrillaId: cuadrillaId,
        },
      });
