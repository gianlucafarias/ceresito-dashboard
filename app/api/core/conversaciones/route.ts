...
function logConversaciones(meta: {
  target: Target;
  source: string;
  status: number;
  url: string;
  ...
  console.log('[core-api/conversaciones]', JSON.stringify(meta));
}
...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${CONVERSACIONES_PATH}${query}`;
}
...
async function fetchLegacyConversaciones(request: NextRequest): Promise<Response> {
...
async function fetchV1Conversaciones(request: NextRequest): Promise<Response | null> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (target === 'legacy') {
      const legacyResponse = await fetchLegacyConversaciones(request);
    ...
    const v1Response = await fetchV1Conversaciones(request);
    if (!v1Response) {
      logConversaciones({
        target,
      ...
      return jsonWithSource(
        {
          ...
          message: 'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 conversaciones',
        },
        500,
        'v1_config_error'
      );
