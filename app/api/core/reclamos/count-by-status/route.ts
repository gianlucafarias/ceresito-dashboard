...
const RECLAMOS_COUNT_BY_STATUS_PATH = '/reclamos/count-by-status';
...
function logReclamosCountByStatus(meta: {
  target: Target;
  source: string;
  status: number;
  url: string;
  ...
  console.log('[core-api/reclamos/count-by-status]', JSON.stringify(meta));
}
...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${RECLAMOS_COUNT_BY_STATUS_PATH}${query}`;
}
      ...
      return jsonWithSource(
        {
          ...
          message: 'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 reclamos/count-by-status',
        },
        500,
        'v1_config_error'
      );
