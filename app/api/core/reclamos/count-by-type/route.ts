...
const RECLAMOS_COUNT_BY_TYPE_PATH = '/reclamos/count-by-type';
...
function logReclamosCountByType(meta: {
  target: Target;
  source: string;
  status: number;
  url: string;
  ...
  console.log('[core-api/reclamos/count-by-type]', JSON.stringify(meta));
}
...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${RECLAMOS_COUNT_BY_TYPE_PATH}${query}`;
}
      ...
      return jsonWithSource(
        {
          ...
          message: 'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 reclamos/count-by-type',
        },
        500,
        'v1_config_error'
      );
