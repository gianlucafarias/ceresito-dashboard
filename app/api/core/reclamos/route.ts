...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${RECLAMOS_PATH}${query}`;
}
...
async function fetchLegacyReclamos(request: NextRequest): Promise<Response> {
...
async function fetchV1Reclamos(request: NextRequest): Promise<Response | null> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (target === 'legacy') {
      const legacyResponse = await fetchLegacyReclamos(request);
    ...
    const v1Response = await fetchV1Reclamos(request);
    if (!v1Response) {
      logReclamos({
        target,
