...
const RECLAMOS_COUNT_BY_PRIORITY_PATH = '/reclamos/count-by-priority';
...
function logReclamosCountByPriority(meta: {
  target: Target;
  source: string;
  status: number;
  url: string;
  ...
  console.log('[core-api/reclamos/count-by-priority]', JSON.stringify(meta));
}
...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${RECLAMOS_COUNT_BY_PRIORITY_PATH}${query}`;
}
      ...
      return jsonWithSource(
        {
          ...
          message: 'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 reclamos/count-by-priority',
        },
        500,
        'v1_config_error'
      );
