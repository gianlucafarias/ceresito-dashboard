...
const V1_PATH = '/history/human-handoff';
...
function logHumanHandoff(meta: {
  source: string;
  status: number;
  url: string;
}) {
  if (!shouldLogDebug()) return;
  console.log('[core-api/history/human-handoff]', JSON.stringify(meta));
}
...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    contactId <= 0 ||
    (action !== 'take' && action !== 'release')
  ) {
    return jsonWithSource(
      {
        success: false,
        error: 'validation_error',
        message: 'contactId y action (take|release) son requeridos',
      },
      400,
      'validation_error',
    );
  ...
  try {
    ...
    logHumanHandoff({ source: 'v1_upstream_unreachable', status: 502, url });
    return jsonWithSource(
      {
        success: false,
  ...
  if (!upstreamResponse.ok) {
    logHumanHandoff({ source: 'v1_error', status: upstreamResponse.status, url });
    return jsonWithSource(
      payload || {
        success: false,
        error: 'v1_request_failed',
        message: 'No se pudo actualizar el handoff',
      },
  ...
  logHumanHandoff({ source: 'v1', status: 200, url });
  return jsonWithSource(
    payload || { success: true, message: 'Handoff actualizado correctamente' },
    200,
    'v1',
  );
}
