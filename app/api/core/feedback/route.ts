...
function logFeedback(meta: {
  ...
  console.log('[core-api/feedback]', JSON.stringify(meta));
}
...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${FEEDBACK_PATH}${query}`;
}
...
async function fetchLegacyFeedback(request: NextRequest): Promise<Response> {
...
async function fetchV1Feedback(request: NextRequest): Promise<Response | null> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (target === 'legacy') {
      const legacyResponse = await fetchLegacyFeedback(request);
    ...
    const v1Response = await fetchV1Feedback(request);
    if (!v1Response) {
      logFeedback({
        target,
      ...
      return jsonWithSource(
        {
          ...
          message: 'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 feedback',
        },
        500,
        'v1_config_error'
      );
