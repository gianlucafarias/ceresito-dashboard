...
const LEGACY_LAST_INTERACTIONS_PATH = '/last-interactions';
const V1_LAST_INTERACTIONS_PATH = '/contacts/last-interactions';
...
function logLastInteractions(meta: {
  target: Target;
  source: string;
  status: number;
  url: string;
  ...
  console.log('[core-api/contacts/last-interactions]', JSON.stringify(meta));
}
...
function buildLegacyUrl(request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(resolveLegacyBaseUrl())}${LEGACY_LAST_INTERACTIONS_PATH}${query}`;
}

function buildV1Url(request: NextRequest): string {
  const query = request.nextUrl.search || '';
  return `${normalizeBaseUrl(resolveV1BaseUrl())}${V1_LAST_INTERACTIONS_PATH}${query}`;
}
...
async function fetchLegacyLastInteractions(request: NextRequest): Promise<Response> {
...
async function fetchV1LastInteractions(request: NextRequest): Promise<Response | null> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (target === 'legacy') {
      const legacyResponse = await fetchLegacyLastInteractions(request);
    ...
    const v1Response = await fetchV1LastInteractions(request);
    if (!v1Response) {
      logLastInteractions({
        target,
      ...
      return jsonWithSource(
        {
          ...
          message:
            'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 contacts/last-interactions',
        },
        500,
        'v1_config_error'
      );
