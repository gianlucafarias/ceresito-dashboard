...
const VISITAS_FLUJO_PATH = '/visitas-flujo';
...
function logVisitasFlujo(meta: {
  ...
  console.log('[core-api/visitas-flujo]', JSON.stringify(meta));
}
...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${VISITAS_FLUJO_PATH}${query}`;
}
...
async function fetchLegacyVisitasFlujo(request: NextRequest): Promise<Response> {
...
async function fetchV1VisitasFlujo(request: NextRequest): Promise<Response | null> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (target === 'legacy') {
      const legacyResponse = await fetchLegacyVisitasFlujo(request);
    ...
    const v1Response = await fetchV1VisitasFlujo(request);
    if (!v1Response) {
      logVisitasFlujo({
        target,
      ...
      return jsonWithSource(
        {
          ...
          message: 'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 visitas-flujo',
        },
        500,
        'v1_config_error'
      );
