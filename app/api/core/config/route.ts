...
function logConfig(meta: {
  ...
  console.log('[core-api/config]', JSON.stringify(meta));
}
...
function buildUrl(baseUrl: string, request: NextRequest): string {
  ...
  return `${normalizeBaseUrl(baseUrl)}${CONFIG_PATH}${query}`;
}
...
async function fetchLegacyConfig(request: NextRequest): Promise<Response> {
...
async function fetchV1Config(request: NextRequest): Promise<Response | null> {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (target === 'legacy') {
      const legacyResponse = await fetchLegacyConfig(request);
    ...
    const v1Response = await fetchV1Config(request);
    if (!v1Response) {
      logConfig({
        target,
      ...
      return jsonWithSource(
        {
          ...
          message: 'CORE_API_ADMIN_KEY or ADMIN_API_KEY is required for v1 config',
        },
        500,
        'v1_config_error'
      );
